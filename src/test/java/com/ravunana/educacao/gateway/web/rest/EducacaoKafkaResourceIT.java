...
import com.ravunana.educacao.gateway.config.KafkaProperties;
...
class EducacaoKafkaResourceIT {
    ...
    void setup() {
        ...
        EducacaoKafkaResource kafkaResource = new EducacaoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/educacao-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/educacao-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
