...
describe('Curso e2e test', () => {
  ...
  it('should load Cursos', async () => {
    ...
    expect(await cursoComponentsPage.getTitle()).to.eq('educacaoApp.pedagogicoCurso.home.title');
  });

  it('should load create Curso page', async () => {
    await cursoComponentsPage.clickOnCreateButton();
    cursoUpdatePage = new CursoUpdatePage();
    expect(await cursoUpdatePage.getPageTitle()).to.eq('educacaoApp.pedagogicoCurso.home.createOrEditLabel');
  ...
  it('should delete last Curso', async () => {
    ...
    expect(await cursoDeleteDialog.getDialogTitle()).to.eq('educacaoApp.pedagogicoCurso.delete.question');
