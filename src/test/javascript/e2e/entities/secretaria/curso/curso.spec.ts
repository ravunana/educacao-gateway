...
describe('Curso e2e test', () => {
  ...
  it('should load Cursos', async () => {
    ...
    expect(await cursoComponentsPage.getTitle()).to.eq('educacaoApp.secretariaCurso.home.title');
  });

  it('should load create Curso page', async () => {
    await cursoComponentsPage.clickOnCreateButton();
    cursoUpdatePage = new CursoUpdatePage();
    expect(await cursoUpdatePage.getPageTitle()).to.eq('educacaoApp.secretariaCurso.home.createOrEditLabel');
  ...
  it('should create and save Cursos', async () => {
    ...
    await promise.all([
      ...
      cursoUpdatePage.setAreaFormacaoInput('areaFormacao')
    ]);
    ...
    expect(await cursoUpdatePage.getAreaFormacaoInput()).to.eq('areaFormacao', 'Expected AreaFormacao value to be equals to areaFormacao');
  ...
  it('should delete last Curso', async () => {
    ...
    expect(await cursoDeleteDialog.getDialogTitle()).to.eq('educacaoApp.secretariaCurso.delete.question');
